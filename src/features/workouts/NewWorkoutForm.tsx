...
export function NewWorkoutForm({ onSave, previousWorkouts, initialWorkout }: Props) {
  ...
  const [durationMinutes, setDurationMinutes] = useState(initialWorkout?.endurance ? String(Math.floor(initialWorkout.endurance.durationSeconds / 60)) : '')
  const [durationSeconds, setDurationSeconds] = useState(initialWorkout?.endurance ? String(initialWorkout.endurance.durationSeconds % 60) : '')
  ...
  const endurance = { distanceKm: Number(distanceKm), durationSeconds: Number(durationMinutes) * 60 + Number(durationSeconds) }
  ...
  function usePreviousWorkout() {
    ...
    else if (previousWorkout.endurance) { setDistanceKm(String(previousWorkout.endurance.distanceKm)); setDurationMinutes(String(Math.floor(previousWorkout.endurance.durationSeconds / 60))); setDurationSeconds(String(previousWorkout.endurance.durationSeconds % 60)); setElevationGainM(previousWorkout.endurance.elevationGainM ? String(previousWorkout.endurance.elevationGainM) : ''); setAverageHeartRate(previousWorkout.endurance.averageHeartRate ? String(previousWorkout.endurance.averageHeartRate) : '') }
  }
  ...
  async function submit(event: FormEvent) {
    ...
    try { setSaving(true); await onSave(workout); setTitle(''); setNotes(''); setExercises([initialExercise()]); setDistanceKm(''); setDurationMinutes(''); setDurationSeconds(''); setElevationGainM(''); setAverageHeartRate(''); setError(null) }
